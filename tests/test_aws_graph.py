...
class AWSGraphTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.aws_usage_single = [{"name": "team1",
                                 ...
                                 {"name": "team1",
                                  "cost": 1.25,
                                  "date": datetime.datetime(2015, 10, 3, 1, 0, 0)}]
        self.aws_usage_multiple_file = "tests/graph/aws_usage_multiple.js"
    ...
    def test_aws_graph_single(self):
            ...
            with open(self.aws_usage_single_file, "r") as f:
                valid_ouput = f.read()

            self.assertEqual(aws_graph.create(usage_list=self.aws_usage_single), valid_ouput)

    def test_aws_graph_multiple(self):
        aws_graph = AWSGraph()

        with open(self.aws_usage_multiple_file, "r") as f:
            valid_ouput = f.read()

        self.assertEqual(aws_graph.create(usage_list=self.aws_usage_multiple), valid_ouput)
